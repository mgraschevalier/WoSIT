...
global addRule
def addRule(target, source=None, command=None, path=None, phony=False):
    return build.addRule(target=target, source=source, command=command, path=path, phony=phony)
